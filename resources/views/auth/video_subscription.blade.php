                        ...
                        <div class="col-lg-7 form-div wow fadeInRight">
                            ...
                            <form class="cd-form" method="POST" action="{{ route('video_subscription',$video_id) }}"
                                ...
                                <div class="form-row wh_class actv_bg" id="first_block">
                                    <small class="col-12 p-0 mb-2"><b>Which Video You want to Subscription</b><span class="text-danger">*</span></small>
                                    <div class="form-group col-sm-12">
                                        <select name="class" class="form-control" id="class_wise">
                                            <option value="" selected>Select Video</option>
                                            @foreach ($videos as $video)
                                                <option value="{{ $video->id }}" @if ($video->id == $video_id)
                                                    selected
                                            @endif>{{ \Illuminate\Support\Str::limit($video->title,50) }}</option>
                                            @endforeach
                                        </select>
                                        <div class="error" style="color : red;">Please Fill This field.</div>
                                    </div>
